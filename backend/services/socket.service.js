...
function connectSockets(http, session) {
   ...
   gIo.on('connection', socket => {
      ...
      socket.on('chat newMsg', msg => {
         ...
         addMsgToToyHistory(msg, socket.topic)
      })
      socket.on('user-watch', userId => {
         socket.join(userId)
      })
      socket.on('typing', ({ isTyping, username }) => {
         // gIo.to(socket.topic).emit('display', { isTyping, username })
         socket.broadcast.emit('display', { isTyping, username })
      })
   })
}
...
function addMsgToToyHistory(msg, toyId) {
   toyService.addMsgToToyHistory(msg, toyId)
}
module.exports = {
   connectSockets,
   emitToAll,
